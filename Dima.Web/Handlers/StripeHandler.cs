...
public class StripeHandler(IHttpClientFactory httpClientFactory) : IStripeHandler
{
    ...
    public async Task<Response<CreateSessionResponse?>> CreateSessionAsync(CreateSessionRequest request)
    {
        ...
        return await result.Content.ReadFromJsonAsync<Response<CreateSessionResponse?>>()
               ?? new Response<CreateSessionResponse?>(null, 400, "Falha ao criar sessão no stripe");
    }
    ...
    public async Task<Response<List<StripeTransactionResponse>>> GetTransactionsByOrderNumberAsync(GetTransactionsByOrderNumberRequest request)
    {
        var result = await _client
            .PostAsJsonAsync($"v1/payments/stripe/{request.Number}/transactions", request);
